...
# using the submodule at third_party/ANTfrastructure to bootstrap a CALLER-CHOSEN
...
#                    image, composed by ANTfrastructure's
...
ANTFRASTRUCTURE_SCRIPTS_DIR="${ANTFRASTRUCTURE_DIR}/linux/scripts"
...
# The tag is NOT spelled here. ANTfrastructure's linux/scripts/ci-image-ref.sh
...
# the real message - the shape antfrastructure_source and antfrastructure_exec use.
if [ -z "${IMAGE:-}" ]; then
...
if [ ! -f "${ANTFRASTRUCTURE_SCRIPTS_DIR}/05-frameworks/flutter/setup-flutter.sh" ]; then
  echo "Error: ${ANTFRASTRUCTURE_SCRIPTS_DIR}/05-frameworks/flutter/setup-flutter.sh not found." >&2
  echo "Did you run 'git submodule update --init third_party/ANTfrastructure'?" >&2
  exit 1
fi
...
exec "${ENGINE}" run --rm -it \
  ...
  -v "${ANTFRASTRUCTURE_SCRIPTS_DIR}:/opt/kataglyphis-scripts:ro" \
  -w /workspace \
  "${IMAGE}" \
